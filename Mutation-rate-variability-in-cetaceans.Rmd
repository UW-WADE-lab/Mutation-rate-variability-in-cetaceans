...
Annual mutation rates varied significantly among chromosomes (Kruskal-Wallis p = `r round(kw_chrom$p.value, digits = 28)`). The range in mutation rates across chromosomes within each species (mean range = `r round(chrom_rate_range$mean, digits = 11)`) was larger than the range in whole genome mutation rates across all species included in the study (`r round(species_rate_range$ratediff, digits = 12)`. Chromosome-level mutation rate were significantly higher in at least half pairwise comparisons in chromosomes 15, 16, and 19. The mutation rate was significantly lower in chromosome X than all other chromosomes (Figure 3). Broadly, relative patterns in mutation rate were consistence across chromosomes, with odotoncete mutation rates higher than mysticete mutation rates in all chromosomes, while the Amazon river dolphin and long-finned pilot whale remained lower than all species across all chromosomes.
...
Our study includes one species - the blue whale - for which pedigree-based mutation rate has also been estimated [@suarez-menendez_etal23]. Interestingly, the estimate of generational mutation rate most similar to pedigree-based estimates was obtained when blue whale sequence data was were aligned to the deeply-diverged hippopotamus ($\mu_{ped}$ = 1.24e-8, CI: 0.85-1.63e-8; mean hippo $\mu_{phy}$ = `r whole_mrate_data %>% filter(abbrev == "Bmus" & reference == "HipAmp" &time_step == "gen") %>% summarise(mean = mean(rate)) %>% pull(mean)`). All estimates of generational whole genome mutation rate in blue whales aligned to the North Atlantic right whale, regardless of divergence date or ancestral heterozygosity, were within the 95% confidence interval of the estimate from Suarez-Menendez et al. [-@suarez-menendez_etal23]. When aligned to the pygmy sperm whale, generational mutation rate estimates (mean $\mu_{phy}$ = `r whole_mrate_data %>% filter(abbrev == "Bmus" & reference == "Kbre" &time_step == "gen") %>% summarise(mean = mean(rate)) %>% pull(mean)`)) were just below the 95% confidence interval from Suarez-Menendez et al. [-@suarez-menendez_etal23]. Given that the estimates in Suarez-Menendez et al. [-@suarez-menendez_etal23] were made using only one pedigree, it is possible that adding additional pedigrees will increase variability in that estimate of mutation rate.
