...
**Inclusion of coding vs non-coding regions:** We indexed the genomic position of coding and non-coding regions using a bed file generated from the annotated pygmy sperm whale reference genome, extracted SNP data in each region, and estimated mutation rates ($\mu$) in coding and non-coding regions of each genome included in the study. We used a Shapiro-Wilk normality test to determine that this mutation rate dataset is normally distributed (p = `r round(coding_normality$p.value, digits = 2)`). We then used an ANOVA to compare mutation rate estimates between these two regions across all fourteen species included in the study.

**Maximum read-depth cutoff:** We first grouped all mutations into bins between read depths of 11-500 reads (i.e. 11-20, 21-30, ... 491-500) and calculated the cumulative number of mutations at each depth bin. Following this, we calculated mutation rate ($\mu$) for each species at maximum read depth cutoffs from 10-500. Finally, we use a generalized additive regression to test whether read depth is a significant driver of mutation rate estimates and a Kruskal-Wallis test to estimate the size of the effect of read depth on mutation rate estimates.
...
Similarly, across 5 humpback whale pedigrees, Suarez-Menendez et al. [-@suarez-menendez_etal23] estimated a pedigree-based mutation rate of 1.12 × 10^−8^ (CI: 0.94 × 10^−8^ to 1.30 × 10^−8^). Again, the mean phylogeny-based generational substitution rate estimate for the humpback whales genome to the hippopotamus reference (1.17 x 10^-8^) was in close alignment with the pedigree-based mutation reate estimate. Mean phylogeny-based estimates of substitution rate were an order of magnitude smaller when the humpback whale was aligned to the pygmy sperm whale (5.03 x 10^-9^) or North Atlantic right whale (6.9 x 10^-9^).
